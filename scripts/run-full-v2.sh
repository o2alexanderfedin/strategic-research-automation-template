...
wait_for_sprint_files() {
    ...
    while [ "$(find sprints -name "*.md" -type f 2>/dev/null | wc -l)" -lt "$NUM_OPPORTUNITIES" ] && [ "$retry_count" -lt "$max_retries" ]; do
        local current_count=$(find sprints -name "*.md" -type f 2>/dev/null | wc -l)
    ...
    local sprint_count=$(find sprints -name "*.md" -type f 2>/dev/null | wc -l)
    echo "" | tee -a "$LOG_FILE"
...
run_sprint_execution_phase() {
    ...
    local sprint_num_total=$(find sprints -name "*.md" -type f 2>/dev/null | wc -l)

    for sprint_file in sprints/*.md; do
        if [ -f "$sprint_file" ]; then
            local sprint_num=$(basename "$sprint_file" | grep -oE '^[0-9]+')
            ...
            local sprint_num_padded=$(printf "%02d" "$sprint_num")
...
run_export_phase() {
    ...
    local export_num_total=$(find sprints -name "*.md" -type f 2>/dev/null | wc -l)
...
commit_and_push_to_github() {
    ...
    local sprint_count=$(find sprints -name "*.md" -type f 2>/dev/null | wc -l)
    local report_count=$(find reports -name "*.md" -type f 2>/dev/null | wc -l | tr -d ' ')
    ...
    if [[ "$repo_url" =~ github.com/([^/]+)/([^/]+) ]]; then
        ...
        if ensure_github_cli_ready; then
            ...
            if enable_github_pages "$github_user" "$repo_name" "$branch" "/docs"; then
                echo ""
                local pages_url=$(get_github_pages_url "$github_user" "$repo_name")
